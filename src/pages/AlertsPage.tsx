        ...
        <div className="px-6 py-6">
          {selectedAlert ? (
            ...
            <>
              ...
              {/* Severity + Filter bar */}
              <div className="flex items-center gap-2 mb-5 flex-wrap p-3 bg-card border border-border rounded-xl">
                <button onClick={() => setShowFilters(!showFilters)}
                  className={`flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold border rounded-lg transition-colors ${
                    showFilters ? "border-primary/40 text-primary bg-primary/10 shadow-sm" : "border-border text-foreground hover:bg-muted bg-background"
                  }`}>
                  <Filter className="h-4 w-4" /> Filters
                  {hasActiveFilters && (
                    <Badge className="bg-primary text-primary-foreground text-[9px] ml-1 h-4 px-1.5 rounded-full">
                ...
                {(["critical", "high", "medium", "low", "informational"] as const).map((sev) => {
                  ...
                  return (
                    <button key={sev} onClick={() => setSelectedSeverity(selectedSeverity === sev ? null : sev)}
                      className={`px-3 py-2 text-[11px] font-mono uppercase rounded-lg border transition-all flex items-center gap-1.5 ${
                        selectedSeverity === sev
                          ? `${severityButtonStyles[sev]} font-bold shadow-sm`
                ...
                {hasActiveFilters && (
                  <button onClick={clearFilters}
                    className="px-3 py-2 text-[11px] font-medium text-destructive hover:text-destructive hover:bg-destructive/5 flex items-center gap-1 ml-1 rounded-lg transition-colors">
